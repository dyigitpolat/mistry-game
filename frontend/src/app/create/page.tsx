...
function CreateCaseForm() {
  ...
  const handleGenerate = async () => {
    console.log("🎨 [CaseStudio] Starting generation process...");
    setGenerating(true);
    setError(null);
    ...
    console.log("📝 [CaseStudio] Generation Payload:", payload);

    try {
      console.log("🚀 [CaseStudio] Sending request to AI Weaver...");
      const result = await generateScenario(payload);
      console.log("✅ [CaseStudio] Generation successful!", result);
      router.push(`/case/${result.scenario_id}`);
    } catch (err) {
      console.error("❌ [CaseStudio] Generation Error:", err);
                  ...
                  <div key={idx} className="bg-surface-dark rounded-xl border border-slate-800 p-5 space-y-4">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <span className={`text-xs font-bold px-2 py-1 rounded uppercase tracking-wider ${char.type === "Suspect" ? "bg-red-500/15 text-red-400" : "bg-blue-500/15 text-blue-400"
                          }`}>
                          {char.type}
                        </span>
                ...
                <ReviewSection title={`Cast (${characters.filter(c => c.name.trim()).length} characters)`}>
                  {characters.filter((c) => c.name.trim()).map((c, i) => (
                    <div key={i} className="flex items-center gap-3 py-2">
                      <span className={`text-[10px] font-bold px-1.5 py-0.5 rounded uppercase ${c.type === "Suspect" ? "bg-red-500/15 text-red-400" : "bg-blue-500/15 text-blue-400"
                        }`}>
                        {c.type}
                      </span>
    ...
    <Suspense fallback={
      <div className="min-h-screen bg-background-dark flex items-center justify-center font-display">
        <div className="text-center">
          ...
          <p className="mt-4 text-lg text-text-secondary">Loading Studio...</p>
        </div>
      </div>
    }>
      <CreateCaseForm />

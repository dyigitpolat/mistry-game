...
3. Response `{ world, layout, placement, artifacts, moods }` is converted by `worldToDungeon`.
...
6. `drawRoomBanner` renders room name/description overlay.
...
| `domain/worldToDungeon.js` | Maps backend payload shape to renderer-friendly room objects; infers floor type, builds exit names, passes connection states and AI-inferred moods |
...
| `rendering/drawScene.js` | Main render loop: procedural floor tiles, walls, gates with labels, objects with shadows/labels |
| `rendering/drawObjects.js` | Object/item sprite rendering, labels, drop shadows, selection highlights, opaque-pixel hit testing |
...
| `rendering/drawOverlays.js` | HUD overlays: room name/description banner |
